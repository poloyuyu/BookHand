...
struct union_find {
	...
	\end{lstlisting}
\subsection{Dominator Tree -- $O((|E|+|V|)\log(|V|))$}
	
	\begin{lstlisting}[style=cpp, title={dominatorTree.cpp}]
struct dominator_tree {
    ll n,l,s;
    vector<ll> tin,par,vis_ord, dsu_mn, dsu_par, sdom, dep,rdom,idom;
    vector<bool> vis;
    vector<vector<ll>> up;
    vector<vector<pair<ll,ll>>> up_mn;
    ...
    void get_sdom(vector<vector<ll>> & rev){
        for (ll i = vis_ord.size()-1; i >= 0; i--){
            ll c = vis_ord[i];
            if (c == s) continue;
            for (ll to : rev[c]){
                ll res = find(to);
    ...
    ll get_min(ll v, ll u){
        ll dist = dep[u]-dep[v];
        pair<ll,ll> ret = up_mn[u][0];
        ll w = u;
        for_(i, l)
            if (dist&(1<<i)){
    ...
    dominator_tree(vector<vector<ll>> & adj, vector<vector<ll>> & rev, ll _s = 0, ll logn = 20){
        ...
        idom.resize(n,-1);
        get_sdom(rev);
        up.assign(n,vector<ll>(logn));
        up_mn.assign(n,vector<pair<ll,ll>>(logn));
        init_up();
        get_idom();
    }
};

//aux function, cover dom tree
void dfs(ll &cnt, ll node, vector<vector<ll>> &adj, vector<ll> &tin, vector<ll> &tout){
    tin[node] = cnt++;
    for(ll child : adj[node]) dfs(cnt, child, adj, tin, tout);
    tout[node] = cnt++;
}

//verifiy if u dominates v, for each edge
vector<vector<bool>> Verifiy_u_dom_v(ll n, vector<vector<ll>> &adj, vector<vector<ll>> &rev_adj, vector<pair<ll, ll>> &edges){
    dominator_tree dt(adj, rev_adj);
    vector<vector<ll>> dom_tree(n);
    for_(i, n){
        if(i==0) continue;
        if(dt.vis[i]) dom_tree[dt.idom[i]].push_back(i);
    }
    vector<ll> tin(n, 0), tout(n, 0); ll cnt = 0;
    dfs(cnt, 0, dom_tree, tin, tout);
    vector<vector<bool>> dom_matrix(n, vector<bool>(n, false));
    for_(i, edges.size()){
        auto &[u,v] = edges[i];
        if((tin[u]<=tin[v] && tout[v]<=tout[u])) dom_matrix[u][v] = true;
    }
    return dom_matrix;
}
	\end{lstlisting}
...
int fenwick_tree::kth(int k) {
    ...
    \subsection{Fenwick Tree 2D-- $O(\log n)$}
    
	\begin{lstlisting}[style=cpp, title={fenwick$\_$tree$\_$2d.cpp}]
...
struct AhoCorasick {
	...
	\subsection{LIS of length K-- $O(nk \log n)$}
	\begin{lstlisting}[style=cpp, title={LIS$\_$lenght$\_$k.cpp}]
    //k = searched length, m = compressed size
    vector<fenwick_tree> bit(k + 1, fenwick_tree(m));

    ll answer = 0;
    for (ll x : v) {
        ll value = compress(x); //or already compressed
        vector<ll> current(k+1);
        current[1] = 1;
        for (ll len = 2; len <= k; len++) {
            current[len] = bit[len-1].sum(value-1);
        }
        for (ll len = 1; len <= k; len++) {
            bit[len].add(value, current[len]);
        }
        answer += current[k];
    }
	...
	\subsection{LIS elements participate of Optimal solutions-- $O(n \log n)$}
	\begin{lstlisting}[style=cpp, title={LIS$\_$elements$\_$optimal.cpp}]
    /*
    L[i]: longest strictly increasing subsequence ending at i
    R[i]: longest strictly increasing subsequence starting at i
    Theroem: best sol force through x = best before x+best after x-overlap on x.
    So if optimal==L[i]+R[i]-1(overlap) -> index i its part of an optimal solution
    layer: number of LIS-participating elements
    */
    fenwick_max<ll> bit(m + 1);
    fenwick_max<ll> bit2(m + 1);

    vector<ll> L(n), R(n);

    for (ll i = 0; i < n; i++) {
        L[i] = bit.query(v[i] - 1) + 1;
        bit.update(v[i], L[i]);
    }

    for (ll i = n - 1; i >= 0; i--) {
        R[i] = bit2.query(m - v[i]) + 1;
        bit2.update(m - v[i] + 1, R[i]);
    }

    ll k = *max_element(L.begin(), L.end());

    vector<ll> layer(k + 1, 0);

    for (ll i = 0; i < n; i++) {
        if (L[i] + R[i] - 1 == k) layer[L[i]]++;
    }
    \end{lstlisting}
